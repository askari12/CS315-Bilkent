...
def createNameMap(accounts):
    accountsDict = {}
    for (index, account) in enumerate(accounts):
        if accountsDict.get(accounts[index].getName(), []):
            accountsDict[accounts[index].getName()] = [index] + accountsDict[accounts[index].getName()]
        else:
            accountsDict[accounts[index].getName()] = [index]
    return accountsDict
